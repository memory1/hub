...
def count_by_person(foundin_id, phaselist):
    ...
    print(phaselist)
    if len(phaselist) > 1:
        sql = """select login_name,count(reporter) from bugs,profiles where bugs.reporter = profiles.userid  and bugs.found_in_version_id= '{0}' and bugs.found_in_phase_id in {1} group by bugs.reporter""".format(
            foundin_id, phaselist)
    else:
        sql = """select login_name,count(reporter) from bugs,profiles where bugs.reporter = profiles.userid  and bugs.found_in_version_id= '{0}' and bugs.found_in_phase_id= '{1}' group by bugs.reporter""".format(
            foundin_id, phaselist[0])
    cursor.execute(sql)
...
def bug_cycle(bug_id):
    ...
    result_json = json.dumps(update, default=json.default)
    return result_json
...
def getFoundinPhase(foundin_id, foundin_phase):
    ...
    if len(foundin_phase) > 1:
        ...
        sql = """select id from phases where name = '{0}' and  version_id = '{1}' """.format(foundin_phase[0],
                                                                                             foundin_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result)
...
def getCountbyPhase(foundin_id, foundin_phase):
    ...
    if len(foundin_phase) > 1:
        sql = """select count(*) from bugs where found_in_phase_id in {0} and  found_in_version_id = '{1}' and product_id = '18' and category_id not in {2}""".format(
            foundin_phase, foundin_id, i18nbug)
    else:
        sql = """select count(*)  from bugs where found_in_phase_id = '{0}' and  found_in_version_id = '{1}' and product_id = '18' and category_id not in {2}""".format(
            foundin_phase[0], foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
    return result
...
def getBugbyDate(foundin_id):
    ...
    sql = """select creation_ts, bug_id from bugs where found_in_version_id = '{0}' and product_id = '18' and category_id not in {1} and cf_type = 'Defect'""".format(
        foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result, default=json.default)
    return result_json
...
def getRegressionBug(foundin_id):
    ...
    cursor = bzdb_conn.cursor()
    i18nbug = ('2451', '1800', '1742', '736')
    sql = """select creation_ts, bug_id,cf_regression from bugs where found_in_version_id = '{0}' and product_id = '18' and category_id not in {1} and cf_type = 'Defect' and short_desc NOT LIKE '%[i18N%' and cf_regression = 'Yes'""".format(
        foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result, default=json.default)
...
def getBugbyDateforTeam(foundin_id):
    ...
    sql = """select login_name, creation_ts, bug_id from bugs, profiles where bugs.assigned_to = profiles.userid and found_in_version_id = '{0}' and product_id = '18' and category_id not in {1}  and bug_severity in ('serious', 'minor', 'cosmetic') and priority in ('P0', 'P1') and cf_type = 'Defect' and short_desc NOT LIKE '%[i18N%'""".format(
        foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
...
def getBugbyDateforReportTeam(foundin_id):
    ...
    cursor = bzdb_conn.cursor()
    i18nbug = ('2451', '1800', '1742', '736')
    sql = """select login_name, creation_ts, bug_id from bugs, profiles where bugs.reporter = profiles.userid and found_in_version_id = '{0}' and product_id = '18' and category_id not in {1}""".format(
        foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result, default=json_util.default)
...
def getAllAssignee(foundin_id):
    ...
    cursor = bzdb_conn.cursor()
    i18nbug = ('2451', '1800', '1742', '736')
    sql = """select login_name from bugs, profiles where bugs.assigned_to = profiles.userid and found_in_version_id = '{0}' and product_id = '18' and category_id not in {1} and bug_severity in ('critical', 'catastrophic') """.format(
        foundin_id, i18nbug)
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result)
...
def getBugbyDateandPro(foundin_id, severity):
    ...
    cursor.execute(sql)
    result = cursor.fetchall()
    result_json = json.dumps(result, default=bson.json_util.default)
    return result_json
